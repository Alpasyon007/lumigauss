...
class SunModel(torch.nn.Module):
    """
    An enhanced learnable sun model that uses physical sun positions from metadata.
    ...
    - sky_zenith: Per-image zenith sky color [3] (overhead)
    - sky_horizon: Per-image horizon sky color [3]
    - residual_sh: Per-image residual SH coefficients for additional environmental details

    This provides a physically-grounded sun position while allowing the model to learn
    atmospheric scattering, indirect lighting, and other environmental effects.
    """

    def __init__(self, sun_data: Dict, image_names: list, sh_degree: int = 2,
                 device: str = "cuda", use_residual: bool = True):
        """
        Initialize the sun model.
        ...
        Args:
            ...
            use_residual: Whether to use learnable residual SH coefficients.
        """
        super().__init__()
        ...
        # Learnable sun intensity per image (initialized higher for direct sunlight)
        # Shape: [n_images, 3] for RGB
        self.sun_intensity = nn.Parameter(
            torch.ones(self.n_images, 3, device=device) * 5.0
        )

        # Learnable sky colors - zenith (overhead) and horizon
        # This creates a gradient sky model
        self.sky_zenith = nn.Parameter(
            torch.ones(self.n_images, 3, device=device) * 0.5
        )
        self.sky_horizon = nn.Parameter(
            torch.ones(self.n_images, 3, device=device) * 0.2
        )

        # Learnable residual SH coefficients for additional environmental details
        # This allows learning indirect lighting, reflections, etc.
        if use_residual:
            self.residual_sh = nn.Parameter(
                torch.zeros(self.n_images, 3, self.n_coeffs, device=device)
            )

    def compute_sun_sh(self, direction: torch.Tensor, intensity: torch.Tensor) -> torch.Tensor:
        """
        Compute SH coefficients for directional sun light.

        Uses a soft directional model (small angular extent) rather than a pure delta.
        """
        device = direction.device
        direction = direction / (torch.norm(direction) + 1e-8)
        x, y, z = direction[0], direction[1], direction[2]
        ...
        sh_rgb = torch.zeros(3, self.n_coeffs, device=device)

        for ch in range(3):
            sun_int = intensity[ch]

            # L=0: Some DC contribution from sun
            sh_rgb[ch, 0] = sun_int * 0.5 / c1

            if self.sh_degree >= 1:
                # L=1: Strong directional component
                sh_rgb[ch, 1] = sun_int * y / c2
                sh_rgb[ch, 2] = sun_int * z / c2
                sh_rgb[ch, 3] = sun_int * x / c2

            if self.sh_degree >= 2:
                # L=2: Angular sharpness
                sh_rgb[ch, 4] = sun_int * x * y * 0.5 / c3
                sh_rgb[ch, 5] = sun_int * y * z * 0.5 / c3
                sh_rgb[ch, 6] = sun_int * (3*z*z - 1) * 0.3 / c4
                sh_rgb[ch, 7] = sun_int * x * z * 0.5 / c3
                sh_rgb[ch, 8] = sun_int * (x*x - y*y) * 0.25 / c5

        return sh_rgb

    def compute_sky_sh(self, zenith: torch.Tensor, horizon: torch.Tensor) -> torch.Tensor:
        """
        Compute SH coefficients for a gradient sky model.

        Creates a sky that interpolates between zenith and horizon colors.
        """
        ...
        for ch in range(3):
            # DC component: average of zenith and horizon
            avg = (zenith[ch] + horizon[ch]) * 0.5
            sh_rgb[ch, 0] = avg / c1

            if self.sh_degree >= 1:
                # Y_1^0 (z component): gradient from horizon to zenith
                # Positive z = zenith, so positive coefficient means brighter zenith
                gradient = (zenith[ch] - horizon[ch]) * 0.5
                sh_rgb[ch, 2] = gradient / c2

        return sh_rgb

    def forward(self, image_idx: int) -> torch.Tensor:
        """
        Compute SH coefficients for a specific image.
        ...
        Returns:
            SH coefficients [3, (sh_degree+1)^2]
        """
        direction = self.sun_directions[image_idx]
        intensity = torch.clamp(self.sun_intensity[image_idx], min=0.01)
        zenith = torch.clamp(self.sky_zenith[image_idx], min=0.01)
        horizon = torch.clamp(self.sky_horizon[image_idx], min=0.01)

        # Compute sun contribution
        sun_sh = self.compute_sun_sh(direction, intensity)

        # Compute sky contribution
        sky_sh = self.compute_sky_sh(zenith, horizon)

        # Combine sun and sky
        total_sh = sun_sh + sky_sh

        # Add residual SH if enabled
        if self.use_residual:
            total_sh = total_sh + self.residual_sh[image_idx]
    ...
    @property
    def sky_color(self):
        """For backward compatibility - return sky_zenith as sky_color."""
        return self.sky_zenith


class DirectionalSunModel(torch.nn.Module):
    """
